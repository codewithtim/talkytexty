...
**Linux:** `sudo apt install libasound2-dev libgtk-3-dev libwebkit2gtk-4.1-dev libayatana-appindicator3-dev xdotool libxdo-dev librsvg2-dev desktop-file-utils libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev cmake` (GStreamer is required for frontend audio playback).

**Linux AppImage:** Requires `libfuse2`, `libfuse3-3`, `libsquashfuse-dev`, `patchelf`, `imagemagick`. Build with: `export APPIMAGE_EXTRACT_AND_RUN=1 && pnpm tauri build`
